    ...
    <div className="space-y-6">
      ...
      >
        ...
        <motion.div
          ...
          className="absolute top-1/2 h-[1px] w-1/3 opacity-20"
        ...
        <div className="relative z-10 p-7 md:p-10">
          ...
          <motion.p
            ...
            className="mt-3 text-white/35 text-sm md:text-base max-w-xl leading-relaxed"
          >
              ...
              <Card className="group hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 overflow-hidden border-border/50">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-2">
                    <div className={`h-8 w-8 rounded-lg bg-gradient-to-br ${goal.gradient} flex items-center justify-center shadow-md`}>
                      <goal.icon className="h-3.5 w-3.5 text-white" />
                    </div>
                    <span className="text-[10px] font-bold text-muted-foreground">{pct}%</span>
                  </div>
                  <p className="text-lg font-heading font-extrabold text-foreground">{goal.value}<span className="text-xs text-muted-foreground font-medium">/{goal.target}</span></p>
            ...
            <Card className="group hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden border-border/50 relative">
              ...
              <CardContent className="p-4 relative">
                <div className="flex items-center justify-between mb-3">
                  <div className={`h-11 w-11 rounded-2xl bg-gradient-to-br ${stat.gradient} flex items-center justify-center shadow-lg group-hover:shadow-xl group-hover:scale-110 transition-all duration-300`}>
              ...
              <div className="flex bg-muted rounded-lg p-0.5">
                {(['week', 'month'] as const).map(tab => (
                  <button
                    ...
                    className={`text-[10px] font-bold px-3 py-1 rounded-md transition-all ${activeTab === tab ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
                  >
                    {tab === 'week' ? 'Week' : 'Month'}
                  </button>
                ))}
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-2.5">
              {quickActions.map((action, i) => (
                ...
                >
                  <div className={`absolute inset-0 bg-gradient-to-br ${action.gradient} opacity-0 group-hover:opacity-[0.05] transition-opacity duration-500`} />
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={symptomData} layout="vertical">
              ...
              <Tooltip contentStyle={{ borderRadius: '12px', border: '1px solid hsl(var(--border))', background: 'hsl(var(--card))', boxShadow: '0 12px 32px rgba(0,0,0,0.12)', fontSize: '11px', fontWeight: 600 }} />
              <Bar dataKey="count" fill="url(#barGrad2)" radius={[0, 10, 10, 0]} barSize={18} />
            </BarChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
            ...
            <div className="flex items-center gap-5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1 mt-2.5">
                  ...
                  <span className="text-[10px] text-muted-foreground font-bold ml-1">+{additionalFeatures.length - 6}</span>
                </div>
              </div>
              <motion.div
