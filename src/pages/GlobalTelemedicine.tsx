...
import { Globe, Search, Star, Clock, Video, Languages, MapPin, Calendar, CheckCircle2, Shield, Wifi, Phone, MessageCircle } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { BookingDialog } from '@/components/telemedicine/BookingDialog';
import { MyBookings } from '@/components/telemedicine/MyBookings';
...
const GlobalTelemedicine = () => {
  ...
  const [bookingDoc, setBookingDoc] = useState<typeof specialists[0] | null>(null);
  ...
  return (
    <div className="space-y-6 relative">
      ...
      >
        ...
        <div className="relative z-10 max-w-xl">
          <div className="flex items-center gap-2 mb-3">
            <div className="p-2 rounded-xl bg-primary-foreground/15 backdrop-blur-sm"><Globe className="h-5 w-5" /></div>
            <Badge className="bg-primary-foreground/15 text-primary-foreground border-0 backdrop-blur-sm text-xs">Live Network</Badge>
          </div>
          <h1 className="text-3xl md:text-4xl font-heading font-extrabold tracking-tight mb-2">Global Telemedicine</h1>
          <p className="text-primary-foreground/70 text-sm md:text-base leading-relaxed max-w-md">
            Connect with world-class specialists. Real-time booking, secure video, and instant confirmation.
          </p>
          <div className="flex flex-wrap gap-3 mt-5">
            {[{ icon: Shield, label: 'HIPAA Compliant' }, { icon: Wifi, label: 'HD Video' }, { icon: Languages, label: 'Auto-Translate' }].map(b => (
      ...
      {/* Tabs: Specialists / My Bookings */}
      <Tabs defaultValue="specialists" className="space-y-4">
        <TabsList className="rounded-xl">
          <TabsTrigger value="specialists" className="rounded-lg gap-1.5"><Search className="h-3.5 w-3.5" /> Find Specialists</TabsTrigger>
          <TabsTrigger value="bookings" className="rounded-lg gap-1.5"><Calendar className="h-3.5 w-3.5" /> My Bookings</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="specialists" className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search by name, specialty, or country..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9 rounded-xl border-border/50 bg-card/80" />
            </div>
            <Select value={specialty} onValueChange={setSpecialty}>
              <SelectTrigger className="w-full sm:w-52 rounded-xl border-border/50 bg-card/80"><SelectValue /></SelectTrigger>
              <SelectContent className="rounded-xl">{specialties.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <AnimatePresence mode="popLayout">
              {filtered.map((d, i) => (
                <motion.div key={d.id} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ delay: i * 0.05, duration: 0.3 }}>
                  <Card className="rounded-2xl border-border/40 bg-card/90 backdrop-blur-sm hover:shadow-colored hover:border-primary/20 transition-all duration-300 cursor-pointer group overflow-hidden" onClick={() => setSelectedDoc(d)}>
                    ...
                    <CardContent className="p-5 space-y-4">
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3">
                          <div className={`h-12 w-12 rounded-2xl bg-gradient-to-br ${avatarGradients[i % avatarGradients.length]} flex items-center justify-center text-sm font-bold text-primary-foreground shadow-sm`}>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        <Badge variant="outline" className="text-[10px] gap-1 rounded-lg border-border/50 bg-muted/30"><Star className="h-3 w-3 text-warning fill-warning" /> {d.rating} ({d.reviews})</Badge>
                        <Badge variant="outline" className="text-[10px] gap-1 rounded-lg border-border/50 bg-muted/30"><Languages className="h-3 w-3 text-accent" /> {d.languages.join(', ')}</Badge>
                        <Badge variant="outline" className="text-[10px] gap-1 rounded-lg border-border/50 bg-muted/30"><Clock className="h-3 w-3 text-info" /> {d.timezone}</Badge>
                      </div>
                      <div className="flex items-center justify-between pt-1">
                        ...
                        <Button size="sm" className="text-xs gap-1.5 rounded-xl shadow-sm" onClick={(e) => { e.stopPropagation(); setBookingDoc(d); }}>
      ...
      <Dialog open={!!selectedDoc} onOpenChange={() => setSelectedDoc(null)}>
        <DialogContent className="sm:max-w-md rounded-2xl border-border/40 bg-card/95 backdrop-blur-xl p-0 overflow-hidden">
          {selectedDoc && (() => {
            const idx = specialists.indexOf(selectedDoc);
              ...
              <>
                ...
                <div className="px-6 pt-10 pb-6 space-y-4">
                  <DialogHeader>
                    ...
                    <DialogDescription className="text-xs">{selectedDoc.specialty} · {selectedDoc.country}</DialogDescription>
                  </DialogHeader>
                  <p className="text-sm text-muted-foreground leading-relaxed">{selectedDoc.bio}</p>
                  <div className="grid grid-cols-3 gap-2">
                    <div className="text-center p-3 rounded-xl bg-muted/40"><Star className="h-4 w-4 text-warning mx-auto mb-1 fill-warning" /><p className="text-sm font-bold">{selectedDoc.rating}</p><p className="text-[10px] text-muted-foreground">{selectedDoc.reviews} reviews</p></div>
                    <div className="text-center p-3 rounded-xl bg-muted/40"><Clock className="h-4 w-4 text-info mx-auto mb-1" /><p className="text-sm font-bold">{selectedDoc.timezone}</p><p className="text-[10px] text-muted-foreground">Timezone</p></div>
                    <div className="text-center p-3 rounded-xl bg-muted/40"><Languages className="h-4 w-4 text-accent mx-auto mb-1" /><p className="text-sm font-bold">{selectedDoc.languages.length}</p><p className="text-[10px] text-muted-foreground">Languages</p></div>
                  </div>
                  <div className="flex flex-wrap gap-1.5">{selectedDoc.languages.map(l => <Badge key={l} variant="outline" className="text-xs rounded-lg">{l}</Badge>)}</div>
                  <Button className="w-full rounded-xl gap-2 shadow-sm" onClick={() => { setSelectedDoc(null); setBookingDoc(selectedDoc); }}>
                    <Calendar className="h-4 w-4" /> Book Appointment · {selectedDoc.price}
                  </Button>
                </div>
              </>
            );
      ...
      <BookingDialog
        ...
        gradientClass={bookingDoc ? avatarGradients[specialists.findIndex(s => s.id === bookingDoc.id) % avatarGradients.length] : ''}
        onBooked={() => setBookingsRefresh(r => r + 1)}
      />
    </div>
  );
};
