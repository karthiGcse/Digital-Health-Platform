...
import {
  Scan, Eye, CheckCircle2, Clock, RefreshCw, Upload, Stethoscope,
  Users, AlertCircle, Package, Hash
} from 'lucide-react';
...
const ScanCentre = () => {
  ...
  const loadOrders = async () => {
    try {
      const scanOrders = await getOrdersByType('scan');
      setOrders(scanOrders);
      ...
      const patientIds = [...new Set(scanOrders.map(o => o.patient_id))];
  ...
  const handleUpdateStatus = async (order: HospitalOrderDB, newStatus: string) => {
    try {
      ...
      if (newStatus === 'seen') {
        toast({ title: '✅ Acknowledged', description: 'Scan request acknowledged. Doctor notified.' });
      }
      if (newStatus === 'in_progress') {
        await addNotificationLog({
          ...
          message: `Your ${order.scan_type || 'scan'} is in progress. Please stay in position.`,
        });
        toast({ title: '📡 Scan Started', description: 'Patient notified.' });
      }
      if (newStatus === 'completed') {
        ...
        await addNotificationLog({
          ...
          message: `Your ${order.scan_type || 'scan'} is complete. Results sent to your doctor.`,
        });
        toast({ title: '✅ Scan Complete', description: 'Results uploaded and sent to doctor.' });
      }
      loadOrders();
    } catch (e: any) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">Scan Centre</h1>
            <p className="text-sm text-muted-foreground">X-Ray • MRI • CT Scan • Ultrasound — Auto-loaded from Doctor</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: 'Pending Scans', value: pendingOrders.length, icon: Package, color: 'from-blue-500 to-cyan-500' },
          ...
          { label: 'Total Today', value: allOrders.length, icon: Scan, color: 'from-violet-500 to-purple-500' },
        ].map(s => (
      ...
      <Tabs defaultValue="pending">
          ...
          <div className="space-y-3">
            {pendingOrders.length === 0 ? (
              <Card className="p-12 text-center">
                <Scan className="h-10 w-10 mx-auto text-muted-foreground/30 mb-3" />
                <p className="text-muted-foreground">No pending scans. Requests appear automatically when doctors send them.</p>
              </Card>
            ) : (
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex items-start gap-3">
                          <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-cyan-500 to-blue-500 flex items-center justify-center text-white text-xs font-bold shrink-0">
                            📡
                          </div>
                          <div>
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              <Badge className="bg-cyan-500/15 text-cyan-600 text-[10px]">{order.scan_type || 'Scan'}</Badge>
                            </div>
                            <p className="text-xs text-muted-foreground mt-1">Reason: {order.order_details}</p>
                            <p className="text-[10px] text-muted-foreground">Referred by doctor • Token #{order.token_id?.slice(0, 8)}</p>
                          </div>
                        </div>
        ...
        <TabsContent value="active">
          <div className="space-y-3">
            {activeOrders.length === 0 ? (
              <p className="text-center text-muted-foreground py-8">No scans in progress.</p>
            ) : (
                    ...
                    <CardContent className="p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">{order.scan_type} — {order.order_details}</p>
                          <Progress value={order.status === 'seen' ? 30 : 65} className="h-2 w-48 mt-2" />
                        </div>
                      </div>
                      <div className="space-y-2">
                        <Label className="text-xs">Result Notes / Findings</Label>
                        <Textarea
                      ...
                      <div className="flex gap-2">
                        <Button size="sm" className="bg-emerald-600 text-white hover:bg-emerald-700" onClick={() => handleUpdateStatus(order, 'completed')}>
                          <Upload className="h-4 w-4 mr-1" /> Upload Results & Complete
                        </Button>
                      </div>
                    </CardContent>
                  </Card>
        ...
        <TabsContent value="all">
          <div className="space-y-2">
            {allOrders.length === 0 ? (
              <p className="text-center text-muted-foreground py-8">No scan orders yet.</p>
            ) : (
              allOrders.map(order => (
